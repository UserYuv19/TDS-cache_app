...
# Simulated slow LLM
# ---------------------------
async def fake_llm_response(query: str) -> str:
    await asyncio.sleep(1.2)  # simulate real LLM latency
...
async def query_endpoint(payload: dict):
    ...
    if embedding_texts:
        ...
        sims = cosine_similarity(
            q_vec, vectorizer.transform(embedding_texts)
        )[0]
    ...
    # ---------- Cache miss ----------
    cache_misses += 1
    answer = await fake_llm_response(query)
...
async def root():
    return {
        "status": "ok",
        "service": "AI cache system",
        "endpoints": ["/ (POST)", "/analytics (GET)"]
    }
